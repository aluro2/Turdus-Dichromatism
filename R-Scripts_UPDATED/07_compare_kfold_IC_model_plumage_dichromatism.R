...
kfold_groups <-
  list.files("Results_UPDATED/model_kfold_IC/") %>%
...
kfold_paths <-
  list.files(path = "Results_UPDATED/model_kfold_IC", full.names = T) %>%
  setNames(str_remove(
    list.files("Results_UPDATED/model_kfold_IC/"),
    '_kfold.RDS')
  ) %>%
...
write_tsv(model_kfolds_comparison, "Results_UPDATED/model_kfolds_comparison.tsv")
